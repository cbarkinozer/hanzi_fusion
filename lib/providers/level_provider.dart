...
List<Level> levels(Ref ref) {
  ...
  List<Level> generateLevels() {
    return [
      ...
      // UPDATED: The ranges are now correctly defined and sequential,
      // preventing HSK 0 characters from appearing in HSK 1.
      Level(
        level: 1,
        name: 'HSK 1',
        characterIds: allCharIds.where((id) => id >= 145 && id <= 300).toList()..sort(),
      ...
      Level(
       ...
       Level(
        level: 6,
        name: 'HSK 6',
        characterIds: allCharIds.where((id) => id > 5000).toList()..sort(),
